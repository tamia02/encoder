...
const userHandler = {
  ...
  update: async ({ where, data }: any) => {
    try {
      ...
      const res = await pool.query(
        `UPDATE "User" SET ${setClause}, "updatedAt" = NOW() WHERE "clerkId" = $1 RETURNING *`,
        [where.clerkId || where.id, ...values]
      );
      return res.rows[0];
    } catch (error) {
      const dbData = readJSON();
      const userIdx = dbData.users.findIndex((u: any) => u.clerkId === (where.clerkId || where.id));
      if (userIdx !== -1) {
...
const agentHandler = {
  ...
  count: async () => {
    try {
      const res = await pool.query('SELECT COUNT(*) FROM "Agent"');
      return parseInt(res.rows[0].count);
    } catch (error) {
      return readJSON().agents.length;
    }
  }
};
...
const callHandler = {
  findMany: async ({ where, include, take }: any) => {
    try {
      ...
      const data = readJSON();
      let calls = data.calls.map((c: any) => ({
        ...c,
        agent: data.agents.find((a: any) => a.id === c.agentId),
        contact: data.contacts.find((co: any) => co.id === c.contactId)
      })).sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      
      if (take) calls = calls.slice(0, take);
      return calls;
    }
  },
  count: async () => {
    try {
      const res = await pool.query('SELECT COUNT(*) FROM "Call"');
      return parseInt(res.rows[0].count);
    } catch (error) {
      return readJSON().calls.length;
    }
  }
};
