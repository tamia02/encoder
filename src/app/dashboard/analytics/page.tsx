...
export default async function AnalyticsPage() {
  ...
  const stats = [
    { label: "Total Call Volume", value: totalCalls.toLocaleString(), trend: "+12.5%", positive: true, icon: <PhoneCall className="w-4 h-4" /> },
    { label: "Completion Rate", value: `${completionRate}%`, trend: "+0.4%", positive: true, icon: <Target className="w-4 h-4" /> },
    { label: "Avg. Conversation", value: avgDurationFormatted, trend: "-15s", positive: true, icon: <Clock className="w-4 h-4" /> },
    { label: "Lead Capture ID", value: totalCalls > 0 ? Math.floor(totalCalls * 0.15).toString() : "0", trend: "+3", positive: true, icon: <Zap className="w-4 h-4" /> },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
        ...
        <div className="p-10 rounded-[3rem] bg-neutral-50 border border-neutral-200 flex flex-col shadow-inner">
          ...
          <div className="space-y-8 flex-1">
            {[
              { region: "North America", share: "62%", color: "bg-blue-600" },
              { region: "Middle East (GCC)", share: "24%", color: "bg-blue-400" },
              { region: "Europe", share: "14%", color: "bg-blue-200" },
            ].map((reg, i) => (
